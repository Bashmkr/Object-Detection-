...
model=YOLO('yolov8s.pt') #the model
...
def RGB(event, x, y, flags, param): #action of the mouse 
    if event == cv2.EVENT_MOUSEMOVE :  
        colorsBGR = [x, y]
        print(colorsBGR)
        

cv2.namedWindow('RGB') #intialize the window
cv2.setMouseCallback('RGB', RGB)

cap=cv2.VideoCapture('peoplecount1.mp4')


my_file = open("coco.txt", "r") #collection of 80 different types
data = my_file.read()
